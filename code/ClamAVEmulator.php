...
class ClamAVEmulator extends ClamAV {
	...
	protected function modeOffline() {
		$this->last_exception = new \ClamdSocketException('*EMULATE MODE* No such file or directory "/not-real-root-folder/run/clamav/clamd.ctl"', 2);
		return self::OFFLINE;
	}

	protected function modeInvalid() {
		throw new LogicException('Invalid "mode" config with value "'.Config::inst()->get(__CLASS__, 'mode').'". Use constants provided in '.__CLASS__.' class.');
	}
}